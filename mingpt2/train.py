...
class MyLoader:
  def __init__(self, config, cur_rank, world_size):
...
if __name__ == '__main__':
  ...
  # DATA PREPARATION
  trainloader = MyLoader(cfg, ddp_local_rank, ddp_world_size)
  ...
  for epoch in range(cfg.TRAINING.ITER):
    ...
    if master_process:
      print(f"epoch: {epoch}, loss: {loss_accum.item(): .4f}, lr: {lr:.4e}, gradnorm: {norm:.4f}, time: {total: .4f}, tok/sec: {token_per_sec}")
